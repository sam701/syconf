        ...
        opt(pair(
            delimited(
                ...
                map(
                    ...
                    |(rem, x)| (rem, if x == "+" { MathOp::Add } else { MathOp::Sub })),
                ml_space0,
            ),
            expr_sum,
        )),
    )), |(a, x)| map_math_op(a, x))(input)
}
        ...
        opt(pair(
            delimited(
                ...
                map(
                    ...
                    |(rem, x)| (rem, if x == "*" { MathOp::Mul } else { MathOp::Div })),
                ml_space0,
            ),
            expr_prod,
        )),
...
fn map_math_op<'a>(expr1: ExprWithLocation<'a>, x: Option<((usize, MathOp), ExprWithLocation<'a>)>) -> ExprWithLocation<'a> {
    match x {
        Some(((rest_len, op), expr2)) => Expr::Math(Box::new(MathOperation { expr1, expr2, op })).with_location(rest_len),
        None => expr1
    }
}
...
#[test]
fn test_math() {
    assert_eq!(expr("1 * 2 + 3 * 4"), Ok(("", Expr::Math(Box::new(MathOperation {
