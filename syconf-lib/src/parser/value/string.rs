...
use crate::parser::{ml_space0, ExprWithLocation};
...
fn test_interpolated_string() {
    ...
    assert_eq!(
        interpolated_string("abc"),
        Ok(("", ConfigString::Raw("abc")))
    );
    assert_eq!(
        interpolated_string("abc ${x}"),
        Ok(("${x}", ConfigString::Raw("abc ")))
    );
    assert_eq!(
        interpolated_string("${ x }"),
        Ok((
            "",
            ConfigString::Interpolated(Expr::Identifier("x").with_location(3))
        ))
    );
}
