...
fn boolean(input: &str) -> IResult<&str, bool> {
    map(alt((tag("true"), tag("false"))), |x| x == "true")(input)
}
...
fn hashmap_entry(input: &str) -> IResult<&str, HashMapEntry> {
    map(
        separated_pair(
            alt((
                map(pair(rest_len, identifier), |(rl, id)| {
...
fn test_list() {
    assert_eq!(
        list("[abc3]"),
        Ok(("", vec![Expr::Identifier("abc3").with_location(5),]))
    );
    assert_eq!(
        list("[1, x, 'hello' ]"),
        Ok((
    ...
    assert_eq!(
        ...
        Ok((
            "",
            vec![Expr::Value(ConfigValue::List(vec![
        ...
        Ok((
            ...
            vec![
                HashMapEntry {
                    ...
                    value: Expr::Value(ConfigValue::String(vec![ConfigString::Raw("earth")]))
                        .with_location(49),
                },
                HashMapEntry {
                ...
                HashMapEntry {
                    ...
                    value: Expr::Value(ConfigValue::List(vec![
                        Expr::Identifier("ab3").with_location(5)
                    ]))
                    .with_location(6),
                },
            ]
